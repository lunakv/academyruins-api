...
async def update_from_pending(resource: str):
    upsertQuery = (
        "INSERT INTO redirects (resource, link) "
...
async def set_pending(resource: str, link: str):
    query = (
        "INSERT INTO redirects_pending (resource, link) "
        "VALUES (%s, %s) "
        "ON CONFLICT (resource) "
        "DO UPDATE SET link = EXCLUDED.link"
    )
...
async def fetch_diff(old_set_code: str, new_set_code: str):
    query = (
        "SELECT diffs.creation_day, changes, source.set_name AS source_set, dest.set_name AS dest_set "
...
async def fetch_pending_diff():
    query = "SELECT changes, set_name AS source_set " "FROM cr_diffs_pending JOIN cr ON source_id = cr.id"
    res = await _fetch_one(query, None)
    return res
...
async def fetch_latest_diff_code():
    query = (
        "SELECT source.set_code AS old, dest.set_code AS new "
        "FROM cr_diffs AS diffs "
        "JOIN cr AS source ON source_id = source.id "
        "JOIN cr AS dest ON dest_id = dest.id "
...
async def fetch_cr_diff_metadata():
    query = (
        "SELECT src.set_code AS source_code, dst.set_code AS dest_code, "
...
async def upload_cr_and_diff(rules_json: dict, diff_json: list, rules_file: str):
    newCrQuery = "INSERT INTO cr_pending (creation_day, data, file_name) " "VALUES (%s, %s, %s) " "RETURNING id"
    ...
    newDiffQuery = "INSERT INTO cr_diffs_pending (creation_day, source_id, dest_id, changes) " "VALUES (%s, %s, %s, %s)"
...
async def confirm_pending_cr_and_diff(set_code: str, set_name: str):
    ...
    diffQuery = (
        "INSERT INTO cr_diffs (creation_day, source_id, dest_id, changes) "
