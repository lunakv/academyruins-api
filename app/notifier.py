...
def notify(message, title, url, formatted):
...
def notify_new_cr(link):
    notify(formatted=True, message=f'New CR version is <a href="{link}">available</a> and ready to be parsed', title='Found new CR', url=_refresh_uri)