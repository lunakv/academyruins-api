...
def get_keyword_definition(rule_id):
...
def get_best_rule(rule_id):
    if re.fullmatch(keyword_action_regex, rule_id):
        return rules.get(rule_id + 'a')
    elif re.fullmatch(keyword_regex, rule_id):
        return get_keyword_definition(rule_id)
    else:
